...
def error(message: str = "Error", code: int = 500, details: Optional[Any] = None) -> Dict[str, Any]:
    ...
    from .enterprise_response import ErrorType, ResponseBuilder
...
def paginated(items: Any, total: int, page: int = 1, per_page: int = 25) -> Dict[str, Any]:
    ...
    response = ListResponseBuilder().success("Items retrieved successfully").data(items, page, per_page, total).build()
    return response.dict()
