...
class ResponseBuilder:
    ...
    def add_error(
        ...
        error = ErrorDetail(code=code, message=message, type=error_type, field=field, details=details)
    ...
    def add_business_error(self, message: str, details: Optional[Dict[str, Any]] = None) -> "ResponseBuilder":
        ...
        return self.add_error(
            code="BUSINESS_ERROR", message=message, error_type=ErrorType.BUSINESS_ERROR, details=details
        )
...
def create_conflict_error_response(message: str, details: Optional[Dict[str, Any]] = None) -> BaseApiResponse:
    ...
    return (
        ResponseBuilder()
        .error(message)
        .add_error(code="CONFLICT", message=message, error_type=ErrorType.CONFLICT_ERROR, details=details)
        .build()
    )
...
def create_list_response(
    ...
    return ListResponseBuilder().success(message).data(items, current_page, items_per_page, total_items).build()
...
def create_temperature_response(temperature_data: List[Dict[str, Any]], facility_id: str) -> BaseApiResponse:
    ...
    if total_readings == 0:
        return (
            ResponseBuilder()
            .warning("No temperature readings found")
            .data({"readings": temperature_data, "summary": {"total": 0, "alerts": 0}})
            .build()
        )
    ...
    if alerts > 0:
        return (
            ResponseBuilder()
            .warning(f"Temperature monitoring complete: {alerts} alerts detected")
            .data(
                {
    ...
    return (
        ResponseBuilder()
        .success("Temperature monitoring complete: All readings within normal range")
        .data(
            {
                "readings": temperature_data,
...
class ResponseCache:
    ...
    def get_cache_key(controller: str, action: str, params: Dict[str, Any]) -> str:
        ...
        hash_obj = hashlib.md5(f"{controller}:{action}:{param_str}".encode(), usedforsecurity=False)  # nosec B324
    ...
    def create_cacheable_response(data: Any, cache_key: str, ttl: int = 300) -> Dict[str, Any]:
        ...
        return {"data": data, "cache_key": cache_key, "cache_ttl": ttl, "cached_at": datetime.utcnow()}
